...
lock pitchAxis to ship:facing*R(-90,0,0).
lock pitchVector to pitchAxis:Vector.

lock yawAxis to ship:facing*R(0,90,0).
lock yawVector to yawAxis:Vector.

lock pitchError to VDOT(np,pitchVector).
lock yawError to VDOT(np,yawVector).

print pitchError.
print yawError.

wait until abs(pitchError) < 0.1 and abs(yawError) < 0.1.
